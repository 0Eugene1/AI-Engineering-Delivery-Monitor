...
| **Last updated** | 2026-07-20 (Live E2E: personal PAT auth closed; service accounts still open) |
...
| `[known]` | **Auth type на 8.20.30 = bearer PAT** — Live E2E 2026-07-20: personal `JIRA_TOKEN` → `POST /api/admin/sync/jira` → ~3506 issues (filter 30532). Service account — всё ещё `[TODO]` (§7 / §10) | Личный PAT (proof); Jira-админ для service account | `GET /rest/api/2/myself` + sync; см. [session_log.md](./session_log.md) Live E2E |
...
**Вывод:** транспортный уровень (TLS, таймауты, парсинг реальных Jira-ответов на ошибки) подтверждён на настоящем сервере и совпадает с поведением, которое уже покрыто unit-тестами на mock-сервере — расхождений не найдено. **Superseded 2026-07-20:** авторизованный live sync с personal bearer PAT успешен (~3506 issues); см. [session_log.md](./session_log.md) «Live E2E». Остаётся `[TODO]` на **service account** (не personal PAT) для production. `JiraSmokeTest` по-прежнему доступен для повторного smoke.
...
| Реальный авторизованный прогон (`/myself` → 200) | ✅ **Закрыто 2026-07-20** через Live E2E: `JIRA_AUTH_TYPE=bearer` personal PAT → sync ~3506 issues (filter 30532). Service account — отдельно, §7 |
...
$env:JIRA_AUTH_TYPE = "bearer"          # Live E2E 2026-07-20: bearer PAT confirmed
...
**Вывод:** Task 2.2 (Auth) **закрыта по факту** Live E2E 2026-07-20 (`bearer` personal PAT → sync). Production SoT credentials = service account — всё ещё Discovery §7 / §10.
...
| `[known]` | Personal PAT `read_api` — Live E2E 2026-07-20 sync OK (3 repos) | личный PAT (proof only) | `GET /api/v4/user` + `POST /api/admin/sync/gitlab` |
...
| Статус | Вопрос | Откуда взять | Как проверить |
|---|---|---|---|
| `[known]` | Jira: personal bearer PAT works (Live E2E 2026-07-20) | личный PAT (proof) | sync ~3506 issues |
| `[known]` | GitLab: personal `read_api` PAT works (Live E2E 2026-07-20) | личный PAT (proof) | sync 3 repos |
| `[TODO]` | Jira: **service account** login/PAT + permissions (read) | Jira-админ; заявка в IT | `GET /rest/api/2/myself` |
| `[TODO]` | GitLab: **group/project token** (не personal), scopes | GitLab-админ | `GET /api/v4/user` + чтение MR |
...
> Personal PATs использованы только для Live E2E proof — **не** production SoT. Для prod нужны service account / Group token (строки `[TODO]` выше).
